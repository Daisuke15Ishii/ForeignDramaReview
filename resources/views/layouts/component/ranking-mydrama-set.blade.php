                                ...
                                @auth
                                    @if(empty($score->reviews()->where('user_id',Auth::user()->id)->first()))
                                        <form action="{{ route('my_drama_set') }}" method="POST">
                                            ...
                                            <input type="hidden" name="drama_id" value="{{ $score->drama()->first()->id }}">
                                    ...
                                    @else
                                        <form action="{{ route('my_favorite_set') }}" method="POST">
                                            ...
                                            <input type="hidden" name="user_id" value="{{ Auth::id() }}">
                                            <input type="hidden" name="drama_id" value="{{ $score->drama()->first()->id }}">
                                            <input type="hidden" name="review_id" value="{{ $score->reviews()->where('user_id', Auth::id())->first()->id }}">
                                            @if($score->drama()->first()->favorites()->where('user_id', Auth::id())->first()->favorite !== 1)
                                                <p class="submit">
